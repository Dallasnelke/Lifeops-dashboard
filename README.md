...
## Current Focus: Atlas Foundation

The active product direction is now the `atlas-mvp/` app, renamed in purpose as **LifeOps Atlas Foundation**. This is the focused foundation for the next version:
...
The older LifeOps dashboard files are preserved in this repository as legacy/future-module source. They should not be deleted yet, but they are no longer the main product surface while the foundation is being rebuilt.
...
- Atlas Command 2.0 with a clearer "what to do next" recommendation, why-now signal, local evidence, expected outcome, decision trace, factor contributions, effort, risk, dependency, confidence, and freshness
...
- Atlas onboarding rebuilt into a minimal Apple-style flow with one question per screen, a vertical Life Map progress timeline, local privacy defaults, real first mission generation, and optional Blueprint under Settings
...
- Atlas MVP bridge that opens the Next.js Atlas companion with a summarized read-only LifeOps context snapshot
...
To view LifeOps from a phone or tablet on the same Wi-Fi, keep the preview server running and open the network URL printed by the server, for example:

```text
http://192.168.x.x:4198/lifeops-dashboard.html
```

If the phone cannot connect, confirm the phone and laptop are on the same Wi-Fi and allow Node.js through Windows Firewall for private networks.
...
- `v2.17.0`: Refined the Atlas-first Home screen so Atlas Command is the clear first action, secondary command controls are tucked into a More menu, Life Score explanation is collapsed until requested, and Atlas Status has its own support card. Improved responsive layering so the app rail, side panels, and mobile layout feel less crowded while preserving storage schema `4`, localStorage key, command logic, Life Tree, onboarding, import/export, and existing user data.
- `v2.16.4`: Updated the local preview server so LifeOps can be viewed from phones and tablets on the same Wi-Fi. The server now listens on the local network and prints laptop plus phone/tablet URLs when started.
